 ...
 * Architecture map
 *
 *   register / configure / enable
 *         |
 *         v
 *   interrupt object table
 *         |
 *         v
 *   dispatch
 *         |
 *         v
 *   handler + nesting frame
 *
 * Manager owns the objects and the nesting state.
 * Hardware is never touched directly.
 */

void nxu_interrupt_manager_init(nxu_u32 interrupt_count);

int nxu_interrupt_register(struct nxu_interrupt *interrupt);
struct nxu_interrupt *nxu_interrupt_lookup(nxu_u32 intid);

int nxu_interrupt_configure(struct nxu_interrupt *interrupt,
...
int nxu_interrupt_set_handler(struct nxu_interrupt *interrupt,
                              void (*handler)(struct nxu_interrupt *),
                              void *context);

int nxu_interrupt_dispatch(nxu_u32 intid);
